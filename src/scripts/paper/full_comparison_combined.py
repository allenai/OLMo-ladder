...
for col in ["Task Loss", "C4 Loss", "TaskCE Loss", "Row Average"]:
    rel_df_7b_display[col] = rel_df_7b_display[col].apply(
...
# Format to add % sign to relative errors
for col in ["Task Loss", "C4 Loss", "TaskCE Loss", "Row Average"]:
    rel_df_13b_display[col] = rel_df_13b_display[col].apply(
        lambda x: f"{x}%" if isinstance(x, (int, float)) else x
    )
...
plt.savefig(
    "src/scripts/paper/figures/intermediate_feature_comparison.png", dpi=300, bbox_inches="tight"
)
plt.show()
