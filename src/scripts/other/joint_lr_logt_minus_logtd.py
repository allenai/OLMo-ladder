...
def main():
    ...
    coefficients = get_coefficients_huber(
        ...
        chinchilla_n_d_lr_logt_minus_logtd_fit,
        grad_chinchilla_n_d_lr_logt_minus_logtd_fit,
        p0=[3.0, 6.0, 0.1, 0.2, 1.0, 0.01, 0.0, 0.0],
        bounds=[
            (None, None),
            (None, None),
            (0, None),
    ...
    for name, data in data_by_name.items():
        ...
        predicted_data_by_name[name] = {
            ...
            "ds": data["ds"],
            "ys": [
                chinchilla_n_d_lr_logt_minus_logtd_fit([n, d, h], coefficients)
    ...
    plt.text(
        x=0.40,
        y=0.90,
        s=f"L(n, d, h) = {A:.2f} / n^{alpha:.2f} + {B:.2f} / d^{beta:.2f} + {E:.2f} - {F:.2f} * (1 - h) / (log(n) + {gamma:.2f}) * (log(d) + {delta:.2f})",
        fontsize=12,
    ...
    plt.suptitle("Fitting loss curves, with LR logt minus logtd correction")
